                    ...
                    <li class="px-6 py-4 hover:bg-gray-50 transition-colors duration-150">
                        <div class="flex items-center justify-between">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="flex items-center space-x-2 text-sm text-gray-500">
                                        ...
                                        @if($promotor->observacoes)
                                            ...
                                            <span class="truncate max-w-xs">{{ Str::limit($promotor->observacoes, 50) }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <button 
                                    wire:click="abrirModalEditar({{ $promotor->id }})"
                                ...
                                <button 
                                    wire:click="deletar({{ $promotor->id }})"
            ...
            <nav role="navigation" aria-label="Navegação de Páginas" class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500">
                        Mostrando <span class="font-medium text-gray-700">{{ $promotores->firstItem() }}</span> até <span class="font-medium text-gray-700">{{ $promotores->lastItem() }}</span> de <span class="font-medium text-gray-700">{{ $promotores->total() }}</span> resultados
                    </p>
                </div>
                <div>
                    <span class="relative z-0 inline-flex rounded-lg">
                        @if ($promotores->onFirstPage())
                            <span class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-400 bg-gray-50 border border-gray-200 cursor-default rounded-l-lg">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                            </span>
                        @else
                            <a href="{{ $promotores->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-l-lg hover:bg-gray-50 hover:text-gray-400">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                            </a>
                        @endif

                        @foreach ($promotores->getUrlRange(1, $promotores->lastPage()) as $page => $url)
                            @if ($page == $promotores->currentPage())
                                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-blue-600 bg-blue-50 border border-blue-200 cursor-default">{{ $page }}</span>
                            @else
                                <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-200 hover:bg-gray-50 hover:text-gray-400">{{ $page }}</a>
                            @endif
                        @endforeach

                        @if ($promotores->hasMorePages())
                            <a href="{{ $promotores->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-r-lg hover:bg-gray-50 hover:text-gray-400">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
                            </a>
                        @else
                            <span class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-400 bg-gray-50 border border-gray-200 cursor-default rounded-r-lg">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
                            </span>
                        @endif
                    </span>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4">
                                <form wire:submit.prevent="salvar">
                                    <!-- Nome -->
                                    <div class="mb-4">
                                        <label for="nome" class="block text-sm font-medium text-gray-700">Nome do Promotor</label>
                                        <input 
                                            wire:model="nome"
                                            type="text" 
                                            id="nome"
                                            ...
                                            placeholder="Digite o nome completo do promotor"
                                        >
                                        @error('nome')
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <select 
                                            wire:model="tipo"
                                            id="tipo"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('tipo') border-red-300 @enderror"
                                        >
                                            <option value="titular">Titular</option>
                                            <option value="substituto">Substituto</option>
                                            <option value="auxiliar">Auxiliar</option>
                                    ...
                                    <div class="mb-4">
                                        <div class="flex items-center">
                                            <input 
                                                wire:model="is_substituto"
                                                type="checkbox" 
                                                id="is_substituto"
                                                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded @error('is_substituto') border-red-300 @enderror"
                                            >
                                            <label for="is_substituto" class="ml-2 block text-sm text-gray-900">
                                                É substituto
                                            </label>
                                        </div>
                                        @error('is_substituto')
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <textarea 
                                            ...
                                            placeholder="Observações adicionais sobre o promotor"
                                        ></textarea>
                                        @error('observacoes')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
