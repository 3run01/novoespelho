    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-1 sm:mt-2 text-sm sm:text-base text-gray-600">Gerencie os períodos de início e fim dos espelhos</p>
            </div>
                    ...
                    <div class="p-6 hover:bg-gray-50 transition-colors">
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-3 mb-3">
                                    <h3 class="text-lg font-semibold text-gray-900">
                                        Período: {{ $periodo->periodo_inicio->format('d/m/Y') }} - {{ $periodo->periodo_fim->format('d/m/Y') }}
                                    </h3>
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            <div class="text-sm text-gray-600">
                                                ...
                                                <div class="mt-1">{{ $periodo->periodo_inicio->format('d/m/Y') }}</div>
                                            </div>
                                            <div class="text-sm text-gray-600">
                                            ...
                                            <div class="text-sm text-gray-600">
                                                ...
                                                <div class="mt-1">{{ $periodo->periodo_inicio->diffInDays($periodo->periodo_fim) + 1 }} dias</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col justify-center">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $statusClass }} w-fit">
                                            {{ $statusText }}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 lg:flex-shrink-0">
                                <button 
                                    wire:click="abrirModalEditar({{ $periodo->id }})"
    ...
    @if($mostrarModal)
        <div class="fixed inset-0 overflow-y-auto" style="z-index: 9998 !important;" x-data="{ show: true }" x-show="show" x-transition>
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-lg px-4 sm:px-6 pt-5 sm:pt-6 pb-4 sm:pb-6 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full mx-4 sm:mx-0" x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    ...
                    <div class="w-full">
                        <form wire:submit.prevent="salvar">
                            <div class="space-y-4">
                                <div>
                                    <label for="periodo_inicio" class="block text-sm font-medium text-gray-700">Data de Início</label>
                                    <input 
                                ...
                                <div>
                                    <label for="periodo_fim" class="block text-sm font-medium text-gray-700">Data de Fim</label>
                                    <input 
