<div class="ml-44 mt-24 w-full max-w-7xl p-4 sm:p-6 bg-white rounded-lg shadow-sm" x-data="municipiosData()">
                    ...
                    <li class="px-6 py-4 hover:bg-gray-50 transition-colors duration-150">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <button 
                                    wire:click="deletar({{ $municipio->id }})"
                                    wire:confirm="Tem certeza que deseja deletar este município?"
            ...
            <nav role="navigation" aria-label="Navegação de Páginas" class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500">
                        Mostrando <span class="font-medium text-gray-700">{{ $municipios->firstItem() }}</span> até <span class="font-medium text-gray-700">{{ $municipios->lastItem() }}</span> de <span class="font-medium text-gray-700">{{ $municipios->total() }}</span> resultados
                    </p>
                </div>
                <div>
                    <span class="relative z-0 inline-flex rounded-lg">
                        ...
                        @else
                            <a href="{{ $municipios->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-l-lg hover:bg-gray-50 hover:text-gray-400">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                            </a>
                        @endif

                        @foreach ($municipios->getUrlRange(1, $municipios->lastPage()) as $page => $url)
                            @if ($page == $municipios->currentPage())
                        ...
                        @if ($municipios->hasMorePages())
                            <a href="{{ $municipios->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-r-lg hover:bg-gray-50 hover:text-gray-400">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4">
                                <form wire:submit.prevent="salvar">
                                    <div>
                                        <label for="nome" class="block text-sm font-medium text-gray-700">Nome do Município</label>
                                        <input 
                                            wire:model="nome"
                                            type="text" 
                                            id="nome"
